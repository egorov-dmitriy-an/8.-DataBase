DROP DATABASE IF  EXISTS seminar4home;
...
WHERE sh.id NOT IN 
	(SELECT  s.id
	FROM shops s	
	JOIN (SELECT c.shops_id FROM cats c WHERE c.name = "Murzik" OR c.name = "Zuza") jn
	ON s.id = jn.shops_id);
...
-- 4.1 Анализы, которые продавались 5 февраля 2020
    SELECT an_name, an_price, ord_datetime
    FROM analysis
    JOIN (SELECT * 
		FROM Orders
		WHERE ord_datetime BETWEEN '2020-02-05 00:00:00' AND '2020-02-05 23:59:59') jn
	ON an_id = jn.ord_an;
    
-- 4.2 Анализы, которые продавались с 6 по 12 февраля 2020
    SELECT an_name, an_price, ord_datetime
    FROM analysis
    JOIN (SELECT * 
		FROM Orders
		WHERE ord_datetime BETWEEN '2020-02-06 00:00:00' AND '2020-12-05 23:59:59') jn
	ON an_id = jn.ord_an;