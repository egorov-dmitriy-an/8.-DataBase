...
со следующей строкой, чтобы получить результат.
...
INSERT sched(train_id, station, station_time)
VALUES
...
SELECT sss.train_id, sss.station, sss.station_time, SUBTIME(sss.time_next, sss.station_time) as time_to_next_station
FROM
(SELECT
		train_id,
		station,
		station_time,
		LEAD(station_time) OVER(PARTITION BY train_id ORDER BY train_id, station_time) AS time_next
	FROM sched) sss; 