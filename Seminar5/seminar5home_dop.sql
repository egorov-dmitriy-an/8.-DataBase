CREATE DATABASE IF NOT EXISTS seminar5home;
USE seminar5home;
...
-- -------------------------------------------
-- 5.1. выбрать всех пользователей, указав их id, имя и фамилию, город и аватарку
-- (используя вложенные запросы)
SELECT id, firstname, lastname, hometown, filename
FROM users
LEFT JOIN 
	(SELECT profiles.user_id, hometown, filename
	FROM profiles
	LEFT JOIN media
	ON profiles.photo_id = media.id) jo1
ON users.id = jo1.user_id;

-- 5.2 Список медиафайлов пользователей, указав название типа медиа (id, filename, name_type)
-- (используя JOIN)
